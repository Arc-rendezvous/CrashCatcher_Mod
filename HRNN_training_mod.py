...
plt.rcdefaults()  ### set the defaults
...
def load_set(videofile):
    ...
    while success: ### while success == True
        ...
        ### if not, pad the end with zeros
            ...
            length = len(frames)
            for i in range (0,99-length):
                #print(i)
                frames = numpy.concatenate((frames,numpy.zeros((1,144,256))))
...
img_filepath = '/home/gunawan/TUBES DSKC/CrashCatcher-master/videos/training/' #### the filepath for the training video set
...
"""
neg_all = glob.glob('/home/gunawan/TUBES DSKC/CrashCatcher-master/videos/training/negative/*.mp4') #### negative examples - ACCV
print (neg_all)
pos_2 = glob.glob('/home/gunawan/TUBES DSKC/CrashCatcher-master/videos/training/positive/*.mp4') #### positive examples - ACCV
print (pos_2)
pos_1 = glob.glob('/home/gunawan/TUBES DSKC/CrashCatcher-master/videos/YTpickles/*.pkl')#### positive examples - youtube
print (pos_1)
pos_all = numpy.concatenate((pos_1, pos_2))
"""
...
"""
print (len(neg_all), len(pos_all), len(all_files))                                 #### print check
print (neg_all)
print (pos_all)
print (all_files)
"""
...
def make_dataset(rand):
    seq1 = numpy.zeros((len(rand), 99, 144, 256),dtype=numpy.float16)   ### create an empty array to take in the data
    for i,fi in enumerate(rand):                    ### for each file...
        ...
        if numpy.shape(t)==(99,144,256):                  ### double check to make sure the shape is correct, and accept
            ...
            print('Image has shape ', numpy.shape(t), 'but needs to be shape', numpy.shape(seq1[0])) ### if exception is raised, explain
...
print (len(x_train), len(x_t1), len(y_train), len(y_t1))
#print (x_train)
#print (x_t1)
#print (y_train)
#print (y_t1)
...
#print (x_train)
#print (y_train)
x_testA = numpy.array(x_t1[(int)(len(x_t1)/2):]); y_testA = numpy.array(y_t1[(int)(len(y_t1)/2):])    #### test set
"""
print (len(x_testA), len(y_testA))
print (x_testA)
print ("""""""")
print (y_testA)
"""
### valid set for model
x_testB = numpy.array(x_t1[:(int)(len(x_t1)/2)]); y_testB = numpy.array(y_t1[:(int)(len(y_t1)/2)])    ### need to be arrays

#print (len(x_testB), len(y_testB))
#print (x_testB)
#print ("""""""")
#print (y_testB)
...
for i in range(0, 30):               ### number of epochs
	...
	print('Countdebug: ' + str(countdebug))
	countdebug = countdebug + 1
        ...
        model.fit(xx, yy,                            ### fit training data
                  ...
                  validation_data=(x_test, y_testB),  ### validation set from up earlier in notebook
                  callbacks=callbacks_list)          ### save if better than previous!

# evaluate
scores = model.evaluate(x_test, y_testB, verbose=0)    ### score model
...
xlabel('False Positive Rate'); plt.ylabel('True Positive Rate')
